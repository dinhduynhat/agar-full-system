...
const sensorCount = 360;
const sensorRange = 1000;
...
if (_.isString(args[0])) {
  ...
  const model = Math.max(Math.min(mins / totalMins, 0.99) ,0) / 3;
  reinforceSetup(model);
  stats = fs.openSync('stats/coord' + mins + '.data', 'w');
  fs.writeSync(stats, "coord, " + mins + ", 100s interval\n");
} else {
  reinforceSetup(0);
}
agarStart();
